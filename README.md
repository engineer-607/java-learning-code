...
├── Java面试复习（完整版）.gmind：Java 面试知识复习笔记
├── 计算机基础.gmind：计算机基础知识复习笔记
└── 项目和分布式复习.gmind：项目与分布式知识复习笔记
```
...
这些值只用于学习演示，不应直接用于生产环境。如果通过 SSH 隧道连接远程数据库，请按照本机端口转发情况调整 JDBC URL 中的本地端口，不要把服务器公网 IP、SSH 凭据、Token 或密钥提交到仓库。
...
- 用于自己的项目时，请根据本地环境修改数据库名称、端口和连接配置，并妥善管理账号、密码与密钥。
